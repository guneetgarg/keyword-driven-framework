...
import com.dish.test.Constants;
import com.dish.xls.read.Xls_Reader;
...
public class ReportUtil {
	...
	public static String reportsDirPath;
	...
	public static void createPath() throws Exception {
		...
		reportsDirPath = System.getProperty("user.dir") + "\\Reports\\"+result_FolderName;
		
		File file = new File(reportsDirPath);
        if (!file.exists()) {
            if (file.mkdir()) {
                System.out.println("Directory is created! "+ reportsDirPath);
            } else {
                System.out.println("Failed to create directory!");
            }
        }
		FileInputStream fs = new FileInputStream(System.getProperty("user.dir")
				+ "//src//com//dish//config//config.properties");
	...
	public static void report() throws Exception {
		...
		try {
			...
			int totalTestSuites = suiteXLS
					.getRowCount(Constants.TEST_SUITE_SHEET);
			...
			for (int currentSuiteID = 2; currentSuiteID <= totalTestSuites; currentSuiteID++) {
				...
				currentTestSuite = suiteXLS.getCellData(
						Constants.TEST_SUITE_SHEET, Constants.SUITE_ID,
						currentSuiteID);
				current_suite_xls = new Xls_Reader(
				...
				// String
				// suiteIndexFile=result_FolderName+"\\"+currentTestSuite+".html";
				// new File(suiteIndexFile).createNewFile();
				// add basic format to suite xls Test
				// FileWriter fstream_suite_index= new
				// FileWriter(suiteIndexFile);
				// BufferedWriter out_suite_index= new
				// BufferedWriter(fstream_suite_index);
				/*
				 * out_suite_index.write("<html><HEAD> <TITLE>"+currentTestSuite+
				 ...
				 * ); out_suite_index.write(environment); out_suite_index.write(
				 ...
				 * ); out_suite_index.write(release); out_suite_index.write(
				 ...
				 * );
				 */
				for (int currentTestCaseID = 2; currentTestCaseID <= current_suite_xls
						.getRowCount(Constants.TEST_CASES_SHEET); currentTestCaseID++) {
					currentTestName = null;
					currentTestDescription = null;
					currentTestRunmode = null;

					currentTestName = current_suite_xls.getCellData(
							Constants.TEST_CASES_SHEET, Constants.TCID,
							currentTestCaseID);
					currentTestDescription = current_suite_xls.getCellData(
							Constants.TEST_CASES_SHEET, Constants.DESCRIPTION,
							currentTestCaseID);
					currentTestRunmode = current_suite_xls.getCellData(
							Constants.TEST_CASES_SHEET, Constants.RUNMODE,
							currentTestCaseID);
					System.out.println(currentTestSuite + " -- "
							+ currentTestName);

					// make the file corresponding to test Steps
					String testSteps_file = reportsDirPath + "\\"
							+ currentTestSuite + "_steps.html";
					new File(testSteps_file).createNewFile();

					int rows = current_suite_xls
							.getRowCount(Constants.TEST_STEPS_SHEET);
					int cols = current_suite_xls
							.getColumnCount(Constants.TEST_STEPS_SHEET);
					FileWriter fstream_test_steps = new FileWriter(
							testSteps_file);
					BufferedWriter out_test_steps = new BufferedWriter(
							fstream_test_steps);
					out_test_steps
					...
					for (int colNum = 0; colNum < cols; colNum++) {
						...
						out_test_steps.write(current_suite_xls.getCellData(
								Constants.TEST_STEPS_SHEET, colNum, 1));
					}
					...
					for (int rowNum = 2; rowNum <= rows; rowNum++) {
						out_test_steps.write("<tr>");
						for (int colNum = 0; colNum < cols; colNum++) {
							String data = current_suite_xls.getCellData(
									Constants.TEST_STEPS_SHEET, colNum, rowNum);
							result_col = current_suite_xls.getCellData(
									Constants.TEST_STEPS_SHEET, colNum, 1)
									.startsWith(Constants.RESULT);
							if (data.isEmpty()) {
								if (result_col)
									data = "SKIP";
								else
							...
							if ((data.startsWith("Pass") || data
									.startsWith("PASS")) && result_col)
								out_test_steps
										.write("<td align=center bgcolor=green><FONT COLOR=#000000 FACE= Arial  SIZE=1>");
							else if ((data.startsWith("Fail") || data
									.startsWith("FAIL")) && result_col) {
								out_test_steps
										.write("<td align=center bgcolor=red><FONT COLOR=#000000 FACE= Arial  SIZE=1>");
								if (suite_result.equals(""))
									suite_result = "FAIL";
							} else if ((data.startsWith("Skip") || data
									.startsWith("SKIP")) && result_col)
								out_test_steps
										.write("<td align=center bgcolor=yellow><FONT COLOR=#000000 FACE= Arial  SIZE=1>");
							else
								out_test_steps
										.write("<td align= center bgcolor=#ffffff><FONT COLOR=#000000 FACE= Arial  SIZE=1>");
					...
					/*
					 * out_suite_index.write(
					 * "<tr><td width=20% align= center><FONT COLOR=#153E7E FACE= Arial  SIZE=2><b>"
					 * ); out_suite_index.write(currentTestName);
					 * out_suite_index.write(
					 * "</b></td><td width=40% align= center><FONT COLOR=#153E7E FACE= Arial  SIZE=2><b>"
					 * ); out_suite_index.write(currentTestDescription);
					 * out_suite_index
					 * .write("</b></td><td width=10% align=center  bgcolor=");
					 * 
					 * if(!currentTestRunmode.equalsIgnoreCase(Constants.RUNMODE_YES
					 * )) out_suite_index.write(
					 * "yellow><FONT COLOR=153E7E FACE=Arial SIZE=2><b>Skip</b></td></tr>"
					 * ); else out_suite_index.write(
					 * "green><FONT COLOR=153E7E FACE=Arial SIZE=2><b>Pass</b></td></tr>"
					 * );
					 */
				}
				...
				out.write("<a href=" + currentTestSuite.replace(" ", "%20")
						+ "_steps.html>" + currentTestSuite + "</a>");
